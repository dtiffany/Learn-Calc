...
(function() {       // #1
    ...
    const page1 = {
        ...
        willyWalk: function() {
            this.count += 1;
            if (this.count > 10) {
                this.distance = Math.min(window.innerWidth * 0.8, 1000);
                this.count = 0;
                this.restartCount += 1;
           ...
           // blueWrap.style.width = this.distance + "px";
            ...
            const disDisplay = this.roundIfNecessary(this.disCount, 11);

            document.querySelector('#distance-display').textContent = `Distance to Bathroom: ${disDisplay} meters`;
